...
public class E4BeanFieldGroup<T> extends FieldGroup {
	...
	public E4BeanFieldGroup(Class<T> beanType, IEclipseContext context) {
		...
		if (getItemDataSource() != null) {
			...
			try {
				...
				throw new BindException(
						"Cannot determine type of propertyId '" + propertyId + "'. Unable to introspect " + beanType,
						e);
			}
		}
	}
	...
	protected Object findPropertyId(java.lang.reflect.Field memberField) {
		...
		if (isNested) {
			...
			if (dataSource != null && dataSource.getItemProperty(nestedPropertyId) != null) {
	...
	protected boolean isBeanValidationImplementationAvailable() {
		...
		BundleContext bc = FrameworkUtil.getBundle(E4BeanFieldGroup.class).getBundleContext();
	 ...
	 * <p>
	 * The returned {@link E4BeanFieldGroup} can be used for further
	 * configuration.
	 * 
	 ...
	 * @param context
	 *            the eclipse context for injection stuff
	 * @return the bean field group used to make binding
	 */
	public static <T> E4BeanFieldGroup<T> bindFieldsUnbuffered(T bean, Object objectWithMemberFields,
			IEclipseContext context) {
		return createAndBindFields(bean, objectWithMemberFields, false, context);
	}
	 ...
	 * @return the bean field group used to make binding
	 */
	public static <T> E4BeanFieldGroup<T> bindFieldsBuffered(T bean, Object objectWithMemberFields,
			IEclipseContext context) {
		return createAndBindFields(bean, objectWithMemberFields, true, context);
	}

	private static <T> E4BeanFieldGroup<T> createAndBindFields(T bean, Object objectWithMemberFields, boolean buffered,
			IEclipseContext context) {
		@SuppressWarnings("unchecked")
		E4BeanFieldGroup<T> beanFieldGroup = new E4BeanFieldGroup<T>((Class<T>) bean.getClass(), context);
		// set the E4 field factory
		beanFieldGroup.setFieldFactory(context.get(FieldGroupFieldFactory.class));
